...
import MeasureCompanyUpdateQ3ActualModal from "../dialogs/measure-company-update-q3-actual/MeasureCompanyUpdateQ3ActualModal";
...
const MeasureTableItem = (props: IMeasureTableItemProps) => {
  ...
  const handleUpdateMeasureProgress = (
    ...
    store.companyMeasure.select(measure); // select measure
    showModalFromId(
      MODAL_NAMES.EXECUTION.COMPANY_MEASURE_UPDATE_Q3_ACTUAL_MODAL
    );
  };
...
const MeasureTable = (props: IMeasureTableProps) => {
  ...
  }, [agreement.quarter3Review.status, role]);
...
const CompanyScorecardQ3Cycle = observer((props: IProps) => {
  ...
  if (agreement.quarter2Review.status !== "approved")
    return (
      <ErrorBoundary>
        <NoScorecardData
          title="Company Q2 performance is not approved."
          subtitle="You cannot view Quarter 3 tab if the Quarter 2 results has not yet been approved."
          instruction="Please ensure that the Quarter 2 results is approved/reviewed."
        />
      </ErrorBoundary>
    );
  ...
  return (
    <ErrorBoundary>
      ...
      <ErrorBoundary>
        ...
        <Modal
          modalId={MODAL_NAMES.EXECUTION.COMPANY_MEASURE_UPDATE_Q3_ACTUAL_MODAL}
        >
          <MeasureCompanyUpdateQ3ActualModal />
        </Modal>
        <Modal
