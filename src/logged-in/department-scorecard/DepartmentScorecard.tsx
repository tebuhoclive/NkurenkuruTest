...
import ReadDepartmentScorecardCommentModal from "../dialogs/read-department-scorecard-comment/ReadDepartmentScorecardCommentModal";
...
const DepartmentScorecard = observer(() => {
  ...
  const meDepartmentID = store.auth.department; // logged-in user's department

  const hasAccess = useMemo(() => {
    return (
      role === USER_ROLES.SUPER_USER ||
      (role === USER_ROLES.EXECUTIVE_USER && departmentId === meDepartmentID)
    );
  }, [departmentId, meDepartmentID, role]);
  ...
  const handleFeedback = () => {
    showModalFromId(
      MODAL_NAMES.EXECUTION.READ_DEPARTMENT_SCORECARD_COMMENT_MODAL
    );
  };
  ...
  return (
    <ErrorBoundary>
      ...
      <ErrorBoundary>
        ...
        >
          <ReadDepartmentScorecardCommentModal agreement={agreement} />
        </Modal>
