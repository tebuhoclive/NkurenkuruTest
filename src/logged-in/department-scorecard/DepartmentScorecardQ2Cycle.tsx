...
import MeasureDepartmentUpdateQ2ActualModal from "../dialogs/measure-department-update-q2-actual/MeasureDepartmentUpdateQ2ActualModal";
...
const MeasureTableItem = (props: IMeasureTableItemProps) => {
  ...
  const handleUpdateMeasureProgress = (
    ...
    store.departmentMeasure.select(measure); // select measure
    showModalFromId(
      MODAL_NAMES.EXECUTION.DEPARTMENT_MEASURE_UPDATE_Q2_ACTUAL_MODAL
    );
  };
...
const MeasureTable = (props: IMeasureTableProps) => {
  ...
  }, [agreement.quarter2Review.status, hasAccess]);
...
const DepartmentScorecardQ2Cycle = observer((props: IProps) => {
  ...
  if (agreement.quarter1Review.status !== "approved")
    return (
      <ErrorBoundary>
        <NoScorecardData
          title="Department Q1 performance is not approved."
          subtitle="You cannot view Quarter 2 tab if the quarter is not yet approved."
          instruction="Please ensure that the Department Quarter 1 has been reviewed, and approved."
        />
      </ErrorBoundary>
    );
  ...
  return (
    <ErrorBoundary>
      ...
      <ErrorBoundary>
        ...
        >
          <MeasureDepartmentUpdateQ2ActualModal />
        </Modal>
        <Modal
