...
const JobCardTableViewMoreRecent = ({
  jobCards,
  handleEdit,
  onView,
  defaultPage = 1,
  ...
  const sortedJobCards = [...jobCards]
    ...
    .filter((jobCard) => {
      // Filter the sorted job cards to include only those less than a day since issuance
      const timeDiff = timeSinceIssuanceArray.find(
        (item) => item.jobCardId === jobCard.id
      )?.timeDiff;

      // Assuming less than a day means 24 hours
...
export default JobCardTableViewMoreRecent;
