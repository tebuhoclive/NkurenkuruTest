...
import DepartmentScorecardQ4ApprovalModal from "../dialogs/department-scorecard-q4-approval/DepartmentScorecardQ4ApprovalModal";
import DepartmentScorecardQ4RejectionModal from "../dialogs/department-scorecard-q4-rejection/DepartmentScorecardQ4RejectionModal";
...
const MeasureTableItem = (props: IMeasureTableItemProps) => {
  ...
  const [newQ4Rating, setNewQ4Rating] = useState<number | null>(
    () => measure.q4Rating
  );
  ...
  const onRate = (value: string | number) => {
    ...
    setNewQ4Rating(_rating);

    if (measure.q4Rating === value) setunSavedChanges(false);
    else setunSavedChanges(true);
  };

  const handleUpdate = async () => {
    const isUpdated = newQ4Rating || measure.quarter4Actual ? true : false;

    try {
      const $measure: IMeasureDepartment = {
        ...measure,
        q4Rating: newQ4Rating,
        isUpdated,
      };
      await api.departmentMeasure.update($measure, ["q4Rating", "isUpdated"]);
...
const DepartmentScorecardQ4Cycle = observer((props: IProps) => {
  ...
  const handleApproval = () =>
    showModalFromId(MODAL_NAMES.EXECUTION.DEPARTMENT_Q4_APPROVAL_MODAL);

  const handleRejection = () =>
    showModalFromId(MODAL_NAMES.EXECUTION.DEPARTMENT_Q4_REJECTION_MODAL);
  ...
  if (
    ...
    return (
      <ErrorBoundary>
        <NoScorecardData
          title="Department Q3 performance is not submitted."
          subtitle="You cannot view Quarter 4 tab if the quarter is not yet submitted."
          instruction="Please ensure that the Department Quarter 3 has been reviewed, and submitted."
        />
      </ErrorBoundary>
    );
  ...
  return (
    <ErrorBoundary>
                            ...
                            <>
                              ...
                              <li>
                                ...
                                >
                                  ...
                                  ></span>
                                  Revert Quarter 4 Reviews for Changes
                                </button>
                              </li>
                            </>
                          )}
      ...
      {/* Modals */}
      <ErrorBoundary>
        <Modal modalId={MODAL_NAMES.EXECUTION.DEPARTMENT_Q4_APPROVAL_MODAL}>
          <DepartmentScorecardQ4ApprovalModal agreement={agreement} />
        </Modal>
        <Modal modalId={MODAL_NAMES.EXECUTION.DEPARTMENT_Q4_REJECTION_MODAL}>
          <DepartmentScorecardQ4RejectionModal agreement={agreement} />
        </Modal>
      </ErrorBoundary>
    </ErrorBoundary>
  );
